...
**IMPORTANT:** `nature_figure_config.py` monkey-patches `plt.savefig` and `Figure.savefig` to force **600 DPI** and output both **PNG + SVG** formats. The old CLAUDE.md claim of "DPI: 200, format: PNG" is **stale** — all current figures use 600 DPI SVG+PNG.
